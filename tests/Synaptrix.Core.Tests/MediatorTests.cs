...
public string EventName { get; set; } }
    ...
    public class CustomNotificationPublisher : INotificationPublisher
    {
        ...
        /// <param name="notification">The notification</param>
        /// <param name="cancellationToken">The cancellation token</param>
        public async ValueTask Publish(IEnumerable<Func<INotification, CancellationToken, ValueTask>> handlerCalls, INotification notification, CancellationToken cancellationToken)
        {
            WasCalled = true;
    ...
    public async Task Send_ShouldInvokeCorrectRequestHandler_AndReturnResponse()
    {
        ...
        services.AddSynaptrixCoreServices(); // Registers IMediator and ISender
    ...
    public async Task Send_ShouldThrowException_WhenNoHandlerFound()
    {
        ...
        // Act & Assert
        var exception = await Assert.ThrowsAsync<InvalidOperationException>(() => mediator.Send(request).AsTask());
    ...
    public async Task Publish_ShouldNotThrowException_WhenNoNotificationHandlersFound()
    {
        ...
        // Act & Assert
        var exception = await Record.ExceptionAsync(() => mediator.Publish(notification).AsTask());
    ...
    // --- NEW TEST: Custom Notification Publisher via AddSynaptrixCoreServices<TNotificationPublisher> ---
    /// <summary>
    ...
    public async Task Publish_ShouldUseCustomNotificationPublisher_WhenRegisteredViaCoreServices()
    {
        ...
        services.AddSynaptrixCoreServices<CustomNotificationPublisher>();
        services.AddSingleton<INotificationHandler<TestNotification>, TestNotificationHandler1>();
        services.AddSingleton<INotificationHandler<TestNotification>, TestNotificationHandler2>();
    ...
    // --- TEST: Custom Notification Publisher via direct registration ---
    /// <summary>
