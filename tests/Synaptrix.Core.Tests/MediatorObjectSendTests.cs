...
namespace Synaptrix.Core.Tests
{
    ...
    public class TestObjectRequestHandler : IRequestHandler<TestObjectRequest, string>
    {
        ...
        public ValueTask<string> Handle(TestObjectRequest request, CancellationToken cancellationToken)
        {
    ...
    public class TestObjectCommandHandler : IRequestHandler<TestObjectCommand>
    {
        ...
        /// <param name="request">The request</param>
        /// <param name="cancellationToken">The cancellation token</param>
        public ValueTask Handle(TestObjectCommand request, CancellationToken cancellationToken)
        {
            WasHandled = true;
    ...
    public class MediatorObjectSendTests
    {
        ...
        public async Task Send_UnregisteredObjectRequest_ShouldThrowInvalidOperationException()
        {
            ...
            Assert.Contains("No handler found for request of type TestObjectRequest", exception.Message);
        }
        ...
        public async Task Send_ObjectRequestWithCancellationToken_ShouldPropagateToken()
        {
            ...
            var ex = await Assert.ThrowsAsync<OperationCanceledException>(() => mediator.Send(request, cts.Token).AsTask());
            Assert.NotNull(ex);
        }
    }
}
